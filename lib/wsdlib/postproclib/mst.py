...
def prim_mst_edges(G, root, weight = 'weight', data = True):
    """Generate edges in a minimum spanning forest of an undirected 
    weighted graph.
    ...
    nodes = G.nodes()
    a, b = nodes.index(nodes[0]), nodes.index(root)
...
def prim_mst(G, root, weight = 'weight'):
    """Return a minimum spanning tree or forest of an undirected 
    weighted graph.
    ...
    T=nx.Graph(nx.prim_mst_edges(G,root,weight=weight,data=True))
    # Add isolated nodes
    if len(T)!=len(G):
        T.add_nodes_from([n for n,d in G.degree().items() if d==0])
    # Add node and graph attributes as shallow copy
